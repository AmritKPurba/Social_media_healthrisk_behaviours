...
********************************************************************************
//SCQ complete case analysis: SAP 1Bc
********************************************************************************

// data02a_SCQ_cc_1Bc_vs1.do: Syntax for SAP 1Ba analysis 
...
*#FINAL: Run the weighted model using the mlogit command (obtaining RRRs)- 1-<30 mins SM use as ref cat- no adjustment 

svy: mlogit cigecig_COcc ib2.smscq_r5Ccc, rrr baselevel


*#FINAL: Run the weighted model using the mlogit command (obtaining RRRs)- 1-<30 mins SM use as ref cat- adjustment 
...
*# Goodness of fit (generalized Hosmer–Lemeshow goodness-of-fit test) [C] Prob > F =0.23: non sig p value provides evidence of an adequately specified model 

svy: mlogit cigecig_COcc ib2.smscq_r5Ccc i.eth_r6Ccc i.famstr_r3Ccc ib5.hhinc_r5Ccc i.hied_CO7Ccc ib6.hiocc_CO6Ccc i.sex_rBcc i.parcursmk_CO2Ccc i.parstyCOcc i.prvcig_rBcc i.anti_COccim i.prvalc_rBcc i.urb_COcc  i.cmage6_3Ccc i.sibshh_5Ccc i.mag12_r4Ccc avg_inpact_COcc cogab_rcc sdqtotal_rnimpcc risk_rcc i.eng_D i.wales_D  i.scot_D ib10.imd_COcc, rrr baselevel

mlogitgof, table

*# Predictive margins [C] RRR for SM use and cig & ecig use are large and mostly statistically significant, check if it actually represents a sizeable change by exploring the probabilities of cig use from our fitted model. Explore predictive margins of the probability of cig use for the levels of SM use (allow us to compare the effects of SM groups whilst controlling for the distribution of other covariates in the groups, eg. the marginal probability for no SM use is the avg probability assuming everyone in the same does not use SM, the margin for 1-<30 mins assumes everyone uses SM for 1-<30) generates model adjusted risks (predicted marginal proportions)

margins i.smscq_r5Ccc, vce(unconditional)

*# Look at the risk differences (average marginal effects) of these marginal probabilities 

margins, vce(unconditional) dydx(i.smscq_r5Ccc)

*# Look at the average adjusted predictions [C] Confidence intervals vary
