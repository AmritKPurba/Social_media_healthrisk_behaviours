...
********************************************************************************
//SCQ complete case analysis: SAP 1Bb
********************************************************************************

// data02a_SCQ_cc_1Bb_vs1.do: Syntax for SAP 1Ba analysis 
// Amrit Purba 10.05.2022

*Syntax for SAP 1Bb 
*Exposure- SCQ 5-category variable (A1-A4) 
*Outcome- E-cigarette use (binary) 
...
*#Generating Archer-Lemeshow test (goodness of fit test) which is a modification of the Hosmer-Lemeshow test that can be used with survey data [C]  Prob > F =0.18: non sig p value provides evidence of an adequately specified model 

svy: logit ecig_rBcc ib2.smscq_r5Ccc i.eth_r6Ccc i.famstr_r3Ccc ib5.hhinc_r5Ccc i.hied_CO7Ccc ib6.hiocc_CO6Ccc i.sex_rBcc i.parcursmk_CO2Ccc i.parstyCOcc i.prvcig_rBcc i.anti_COccim i.prvalc_rBcc i.urb_COcc  i.cmage6_3Ccc i.mag12_r4Ccc i.sibshh_5Ccc avg_inpact_COcc cogab_rcc sdqtotal_rnimpcc risk_rcc i.eng_D i.wales_D i.scot_D ib10.imd_COcc, or baselevel

estat gof

*# Predictive margins [C] odds ratios for SM use and ecig use are large and mostly statistically significant, check if it actually represents a sizeable change by exploring the probabilities of ecig use from our fitted model. Explore predictive margins of the probability of ecig use for the levels of SM use (allow us to compare the effects of SM groups whilst controlling for the distribution of other covariates in the groups, eg. the marginal probability for no SM use is the avg probability assuming everyone in the same does not use SM, the margin for 1-<30 mins assumes everyone uses SM for 1-<30) generates model adjusted risks (predicted marginal proportions). Output suggests a sizable difference in ecig use between those who use SM for 2 hrs or more (30%) compared to those who dont use SM (11%)

margins i.smscq_r5Ccc, vce(unconditional)

*# Look at the risk differences (average marginal effects) of these marginal probabilities. [C] From the output risk difference between no sm use and 1-<30 mins is around -17% but not significant. Whilst for 30min-<1hr and 1-<30 mins risk differnce is 8% and significant, for 1-<2hrs risk difference is 9% and significant, whilst for more than 2 hrs risk difference is 18% and significant. 

margins, vce(unconditional) dydx(i.smscq_r5Ccc)

*# Look at the average adjusted predictions [C] Confidence intervals appear to be similar thus representing similar variance
